...
以下为验证集中 6 个随机样本的分割结果（左：原图，中：真实标注，右：模型预测）：

![分割示例](results/segmentation_examples.png)

模型预测的二值掩码与真实标注高度吻合，细胞核边界定位准确。部分小尺寸细胞核存在轻微漏检，整体分割质量良好。
...
4. **分割效果**：IoU 0.83 / Dice 0.88 在 96x96 分辨率单类别分割任务上表现良好，细胞核定位准确。可视化结果表明模型预测与真实标注高度一致
